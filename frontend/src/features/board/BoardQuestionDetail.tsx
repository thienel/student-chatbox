...
export function BoardQuestionDetail({ subjectId, classId, question, onBack }: Props) {
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-6">
      <button onClick={onBack} className="inline-flex items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground mb-4">
      ...
      <h3 className="text-xs font-medium text-muted-foreground uppercase tracking-wide mt-6 mb-3">
        {question.answerCount} Answers
      </h3>

      {answers.isLoading ? (
        <div className="space-y-2">{Array.from({ length: 2 }).map((_, i) => <Skeleton key={i} className="h-20 rounded-lg bg-muted" />)}</div>
      ) : (
        <div className="space-y-2">
          {(answers.data ?? []).map(a => (
            <div key={a.id} className={cn('bg-card border rounded-lg p-4 flex items-start gap-3', a.isPinned ? 'border-primary' : 'border-border')}>
              <button
                onClick={() => run(() => upvoteAnswer.mutateAsync(a.id), 'Failed to vote.')}
                className={cn('flex flex-col items-center text-xs shrink-0', a.isUpvotedByMe ? 'text-primary' : 'text-muted-foreground hover:text-foreground')}
              >
                <ArrowBigUp className={cn('h-5 w-5', a.isUpvotedByMe && 'fill-primary')} />
                <span className="tabular-nums">{a.upvoteCount}</span>
              </button>
              <div className="min-w-0 flex-1">
                {a.isPinned && (
                  <span className="inline-flex items-center gap-1 text-[11px] text-primary mb-1 font-medium">
                    <CheckCircle2 className="h-3 w-3" /> Pinned by lecturer
                  </span>
                )}
                <p className="text-sm text-foreground whitespace-pre-wrap">{a.body}</p>
                <p className="text-xs text-muted-foreground mt-1.5">{authorLabel(a.authorId)} · {new Date(a.createdAt).toLocaleDateString()}</p>
              </div>
              {isModerator && (
                <div className="flex items-center gap-1 shrink-0">
      ...
      {canAnswer && (
        <div className="mt-4 space-y-2">
          <Textarea
            ...
            maxLength={5000}
            className="bg-card border-border text-foreground placeholder:text-muted-foreground text-sm rounded-md min-h-[80px] focus-visible:ring-1 focus-visible:ring-primary"
          />
          <div className="flex justify-end">
            <Button onClick={handlePost} disabled={createAnswer.isPending || !body.trim()} className="bg-primary text-primary-foreground hover:bg-primary/90 h-8 px-3 text-sm rounded-md">
