  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-6">
      <div className="flex items-center gap-2 mb-1">
        ...
        <h1 className="text-2xl font-heading font-medium text-ink tracking-tight">This Week's Study Plan</h1>
      </div>
      {plan && (
      ...
      {isLoading ? (
        <div className="space-y-2">
          {Array.from({ length: 5 }).map((_, i) => <Skeleton key={i} className="h-20 rounded-lg bg-muted" />)}
        </div>
      ) : !plan ? (
            ...
            >
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  ...
                  {isToday(day.date) && (
                    <span className="text-[10px] uppercase tracking-wide text-primary bg-primary/10 border border-primary/20 rounded px-1.5 py-0.5">Today</span>
                  )}
                </div>
              ...
              {day.tasks.length === 0 ? (
                <p className="text-xs text-muted-foreground">Rest day — no tasks.</p>
              ) : (
                <div className="space-y-1.5">
                  {day.tasks.map((task, i) => {
                    const Icon = taskIcon[task.type]
                    return (
                      <div key={i} className="flex items-start gap-2.5 rounded-md bg-secondary/50 px-3 py-2">
                        <Icon className="h-3.5 w-3.5 text-muted-foreground mt-0.5 shrink-0" />
                        <div className="min-w-0 flex-1">
                          <p className="text-sm text-foreground">{task.title}</p>
                          <p className="text-xs text-muted-foreground mt-0.5">{task.description}</p>
                        </div>
                        <span className="text-xs text-muted-foreground shrink-0 tabular-nums">{task.estimatedMinutes}m</span>
                      </div>
                    )
                  })}
                </div>
