...
import { ManageLecturersDialog } from './ManageLecturersDialog'
...
export default function AdminSubjectsPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-8 py-10">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border/50 bg-muted/20">
                <th className="text-left py-5 px-6 text-xs font-mono font-semibold text-muted-foreground uppercase tracking-wider">
                  Subject
                </th>
                <th className="text-left py-5 px-6 text-xs font-mono font-semibold text-muted-foreground uppercase tracking-wider hidden sm:table-cell">
                  Code
                </th>
                <th className="text-left py-5 px-6 text-xs font-mono font-semibold text-muted-foreground uppercase tracking-wider hidden md:table-cell">
                  Status
                </th>
            ...
            <tbody className="divide-y divide-border/30">
              {subjects.map((s) => (
                ...
                >
                  ...
                  <td className="py-4 px-6 text-muted-foreground text-sm font-mono font-semibold hidden sm:table-cell">
                    {s.code}
                  </td>
                  <td className="py-4 px-6 hidden md:table-cell">
                    <Badge
                      className={cn(
                        'text-[10px] rounded-full uppercase font-mono tracking-wider px-3 py-1 border',
                        s.status === 'active'
                  ...
                  <td className="py-4 px-6 text-right" onClick={(e) => e.stopPropagation()}>
                    <div className="flex items-center justify-end gap-2 opacity-50 group-hover:opacity-100 transition-opacity">
                      ...
                      <Button
                        ...
                        onClick={(e) => {
                          e.stopPropagation()
                          setManageLecturersSubjectId(s.id)
                        }}
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent className="bg-card border border-border/60 rounded-[2rem] shadow-xl p-0 max-w-md overflow-hidden">
          <div className="px-8 py-6 border-b border-border/40 bg-muted/10">
            <DialogTitle className="text-2xl font-serif text-primary-ink">
              Create Subject
            </DialogTitle>
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            <div className="p-8 space-y-5">
              <div className="space-y-2">
                ...
                />
                {errors.code && (
                  <p className="text-xs text-destructive ml-1">{errors.code.message}</p>
                )}
              </div>
              <div className="space-y-2">
                <Label className="text-xs font-mono uppercase tracking-widest text-muted-foreground ml-1">
            ...
            <div className="px-8 py-5 border-t border-border/40 flex justify-end gap-3 bg-muted/10">
              ...
              <Button
                ...
                className="rounded-full font-mono text-xs tracking-wider px-6 shadow-sm"
              >
      ...
      <ManageLecturersDialog
        subjectId={manageLecturersSubjectId}
        open={!!manageLecturersSubjectId}
        onOpenChange={(o) => !o && setManageLecturersSubjectId(null)}
      />
    </div>
  )
}
